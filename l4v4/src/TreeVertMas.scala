import javax.swing.{DefaultListModel, JTextArea}

import scala.collection.mutable.ArrayBuffer

class TreeVertMas[T] extends  Serializable {

  class Elem(var value: T) extends Comparable[TreeVertMas[T]#Elem] with Serializable {
  ...
  def remove(ind: Int): Boolean = {
    if (ind < 0 || ind >= x.size) {
      ...
      while ( {
        ...
        c = index * 2
        if (c + 1 < x.size - 1 && x(c + 1).compareTo(x(c)) < 0) c += 1
        tmp = x(index)
        x.update(index, x(c))
        x.update(c, tmp)
        index = c
  ...
  def printMas(list: DefaultListModel[String]): Unit = {
    list.clear()
    if (x.nonEmpty) {
      var i = 0
      while ( {
  ...
  def printTree(ta: JTextArea): Unit = {
    ta.setText("")
    if (x.nonEmpty) {
